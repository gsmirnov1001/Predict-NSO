...
# ── Загружаем DataFrame лениво (только при первом запросе) ───────────────
import pandas as pd
...
def get_df():
    ...
    if _DF is None:
        ...
        # Читаем только нужные колонки с экономными типами
        usecols = ['geo_lat', 'geo_lon', 'price', 'area', 'rooms',
                   'level', 'levels', 'building_type', 'object_type', 'price_per_m2']
        # Пробуем сначала с price_per_m2, потом без
        try:
            _DF = pd.read_csv(csv_path, usecols=usecols,
                              dtype={'rooms':'int8','level':'int8','levels':'int8',
                                     'building_type':'int8','object_type':'int8'})
        except ValueError:
            usecols2 = [c for c in usecols if c != 'price_per_m2']
            _DF = pd.read_csv(csv_path, usecols=usecols2,
                              dtype={'rooms':'int8','level':'int8','levels':'int8',
                                     'building_type':'int8','object_type':'int8'})
            _DF['price_per_m2'] = (_DF['price'] / _DF['area']).astype('float32')

        _DF['geo_lat'] = _DF['geo_lat'].astype('float32')
        _DF['geo_lon'] = _DF['geo_lon'].astype('float32')
        _DF['price']   = _DF['price'].astype('float32')
        _DF['area']    = _DF['area'].astype('float32')

        col_map = {'geo_lat':'lat','geo_lon':'lon'}
        for old_c, new_c in col_map.items():
            if old_c in _DF.columns and new_c not in _DF.columns:
                _DF = _DF.rename(columns={old_c: new_c})
        print(f"✓ DataFrame загружен: {len(_DF):,} строк, RAM ~{_DF.memory_usage(deep=True).sum()//1024//1024}MB")
    return _DF

# НЕ загружаем при старте — только при первом запросе к карте/похожим
